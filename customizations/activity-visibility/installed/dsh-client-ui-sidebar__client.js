window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		"activityPill": "hHd-Xa_activityPill",
		"activityWaiting": "hHd-Xa_activityWaiting"
		};
		//#endregion
		...
		*/
function SidebarRoot({ collapsed, width, startSession, toggleSidebar, sessionsList, openSession, t, renderSlot }) {
			...
			const [, setActivityVersion] = (0, react.useState)(0);
			(0, react.useEffect)(() => sessionsList === void 0 ? void 0 : sessionsList.subscribe(() => {
				setActivityVersion((v) => v + 1);
			}), [sessionsList]);
			const [activityOpen, setActivityOpen] = (0, react.useState)(false);
			const activity = (() => {
				if (sessionsList === void 0) return { running: 0, pending: 0, items: [] };
				const snap = sessionsList.getSnapshot();
				const childRunning = {};
				for (const s of Object.values(snap.byId)) {
					if (s.origin === "subagent" && s.running === true) childRunning[s.parentId] = (childRunning[s.parentId] ?? 0) + 1;
				}
				let running = 0;
				let pending = 0;
				const items = [];
				for (const s of Object.values(snap.byId)) {
					...
					const wait = s.pendingInteraction !== void 0;
					const run = s.running === true || (childRunning[s.id] ?? 0) > 0;
					if (!wait && !run) continue;
					if (wait) pending += 1;
					else running += 1;
					items.push({ id: s.id, label: s.displayTitle ?? s.title ?? s.id, state: wait ? "warning" : "ongoing" });
				}
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						}), activity.items.length > 0 && (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
							open: activityOpen,
							onClose: () => {
								setActivityOpen(false);
							},
							items: activity.items.map((it) => ({ id: it.id, label: it.label, icon: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.StateDot, { state: it.state }) })),
							onSelect: (id) => {
								setActivityOpen(false);
								if (openSession !== void 0) openSession(id);
							},
							portal: true,
							closeOnPointerLeave: true,
							anchor: (0, react_jsx_runtime.jsxs)("button", {
								type: "button",
								className: clsx(SidebarRoot_module_css_default.activityPill, activity.pending > 0 && SidebarRoot_module_css_default.activityWaiting),
								"aria-label": activity.pending + " waiting, " + activity.running + " running",
								onClick: () => {
									setActivityOpen((v) => !v);
								},
								children: [(0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.StateDot, { state: activity.pending > 0 ? "warning" : "ongoing" }), wide && (0, react_jsx_runtime.jsx)("span", { children: [activity.pending > 0 ? activity.pending + " waiting" : void 0, activity.running > 0 ? activity.running + " running" : void 0].filter(Boolean).join(" · ") })]
							})
						}), 
